---
're-reselect': major
---

Rewrite re-reselect in native TypeScript.

The internals moved from JavaScript with a hand-maintained `index.d.ts` (~4500 lines of manual per-arity type overloads) to native `.ts` sources whose type declarations are generated at build time. Build tooling moved from Rollup + Babel to `tsdown` (rolldown), and the type layer now derives everything from reselect v5-style variadic-tuple inference instead of hand-written overloads.

The runtime value API is unchanged: the same two factories (`createCachedSelector`, `createStructuredCachedSelector`) and six cache classes are exported, with working CJS, ESM and UMD bundles, and `@veksa/reselect` still resolved as a peer dependency.

### Runtime behavior changes

- `resetRecomputations()` now returns `undefined` instead of `0` (aligned with reselect v5). Use `recomputations()` to read the count.
- **A missing `keySelector` now throws when the selector is created**, not on the first call. `createCachedSelector(...)({ cacheObject })` used to build a selector that only failed with a `TypeError` once it was invoked; it now throws `[re-reselect] Missing "keySelector"` from the factory itself. Because selectors are usually declared at module scope, a selector that was already broken this way now fails at import time rather than at first use.
- An invalid `cacheKey` is stringified with `String()` before going into the warning message. A `symbol` cacheKey previously made the warning path itself throw a `TypeError`.
- The cache lookup treats any falsy value returned by a custom `cacheObject.get()` as a miss, rather than only `undefined`. A cache object that returns `null` for a miss now works instead of crashing.
- A caller's options object is copied before use, so reusing one across factory calls no longer double-wraps the `keySelector` a `keySelectorCreator` produced.
- Selector instances default to a size-1 argument cache rather than reselect's `weakMapMemoize`; pass `argsMemoize` explicitly to restore the previous behaviour. Recomputation counts are unaffected.
- The returned selector function is named `selector` (`fn.name` was previously `''`).

The stricter, inferred type layer introduces the following type-level breaking changes, which may appear as compile errors for strict-mode TypeScript consumers:

- The cached selector's call signature is now a precise tuple inferred from the input selectors, instead of `(state, props, ...args: any[])`. Passing excess positional arguments is no longer accepted.
- The cached selector's parameters must match the union of all input selectors' parameters. Under-supplying arguments declared by an input selector is no longer accepted.
- `resetRecomputations()` is now typed as `() => void` (aligned with reselect v5) instead of `() => number`.
- The cached selector no longer advertises reselect `OutputSelector` members it never attaches at runtime (`memoizedResultFunc`, `lastResult`, `dependencyRecomputations`, `resetDependencyRecomputations`, `memoize`, `argsMemoize`). Reading them used to type-check and then crash. They are still present on the selector `getMatchingSelector` returns, which genuinely is a reselect selector.
- **Mixing annotated and unannotated inline input selectors is now a compile error.** reselect v5 cannot contextually type an inline input selector while it is still inferring the input tuple, so `[(state: State) => state.foo, (state) => state.foo]` resolved the second `state` to `any` and propagated that `any` into the combiner arguments and the selector's result. The hand-written per-arity `.d.ts` typed those correctly, so the migration would have silently dropped type safety with no diagnostic at all. The factory now degrades to a one-parameter function whose parameter type is the explanation (`ImplicitAnyStateError`), so the compiler prints the fix. Annotate `state` on every input selector, or use `createCachedSelector.withTypes<State>()`. A uniformly-`any` selector list is still accepted — that shape is a deliberate opt-out.
- The `keySelector` parameter list is now inferred from the input selectors, so it is checked instead of being `(state, ...args: any[])`. Params the input selectors declare are typed for you; a `keySelector` may still declare **extra** params of its own (re-reselect's "cache dimension supplied via `keySelector` only" pattern), and those flow into the selector's own call signature and into `getMatchingSelector`/`removeMatchingSelector`. An extra param you annotate keeps that type; one you leave unannotated is `unknown`, so its uses are checked rather than silently allowed. Arguments that no input selector and no `keySelector` declares are rejected, as is a `keySelector` param contradicting one an input selector declares.
- `keySelectorCreator` now receives `keySelector` as an **optional** property (`keySelector?: TypedKeySelector<InputSelectors>`), because it genuinely is absent when `keySelectorCreator` is used on its own. Existing creators that call it directly need a guard or a non-null assertion:

  ```ts
  // before
  ({ keySelector }) =>
    (state, id) =>
      keySelector(state, id);
  // after
  ({ keySelector }) =>
    (state, id) =>
      keySelector?.(state, id) ?? id;
  ```

- **A `withTypes<State>()` creator now rejects the variadic form of input selectors**, accepting only the array form. reselect v5 cannot apply the contextual state and infer the input tuple out of a variadic rest at the same time, so `createCachedSelector.withTypes<State>()(fn, fn, combiner)` type-checked while silently collapsing the combiner arguments to `never`. That is now "No overload matches this call", pointing at the array form that does work. Plain `createCachedSelector` (no `withTypes`) still accepts both forms.
- `createStructuredCachedSelector` with a very large selectors object (roughly 50+ keys) may fail to type-check with `TS2589` "Type instantiation is excessively deep". This is inherited from reselect v5's own `createStructuredSelector`, which uses the same object-to-tuple type derivation; the previous hand-written `.d.ts` avoided it via non-recursive mapped types. Runtime behavior is unchanged. Workaround: split into smaller structured selectors, or compose plain `createCachedSelector` calls.

### Removed and changed type exports

reselect v5-style inference now replaces the hand-written type overrides, so several exported types were removed or changed. Most mirrored reselect's own types, so re-reselect no longer re-exports them; import them from `@veksa/reselect` directly, since it's a peer dependency.

**Removed. Import from `@veksa/reselect` instead:**

```ts
// before: import type { Selector, OutputSelector } from 're-reselect';
import type { OutputSelector, Selector } from '@veksa/reselect';
```

`Selector`, `OutputSelector`, and `CreateSelectorInstance` (use reselect's `CreateSelectorFunction`).

**Removed. The parametric variants no longer exist.** reselect v5 folds "parametric" into the params tuple of a single `Selector`/`OutputSelector` type, so these have no replacement to import. Express the props via the tuple instead:

| Removed                                     | Replacement                                            |
| ------------------------------------------- | ------------------------------------------------------ |
| `ParametricSelector<S, P, R>`               | `Selector<S, R, [P]>`                                  |
| `OptionalParametricSelector<S, P, R>`       | `Selector<S, R, [P?]>`                                 |
| `OutputParametricSelector<…>`               | let inference produce `OutputSelector<[...inputs], R>` |
| `OutputParametricCachedSelector<…>`         | `OutputCachedSelector<[...inputs], R>`                 |
| `OutputOptionalParametricCachedSelector<…>` | `OutputCachedSelector<[...inputs], R>`                 |
| `ParametricKeySelector<S, P>`               | `KeySelector<S>` / `TypedKeySelector<Inputs>`          |
| `ParametricKeySelectorCreator<…>`           | `KeySelectorCreator<Inputs, Result>`                   |

**Changed arity** (aligned with reselect v5, which keys these on the input-selector array instead of `State`):

- `OutputCachedSelector<S, R, C, D>` → `OutputCachedSelector<InputSelectors, Result>`
- `KeySelectorCreator<S, C, D>` → `KeySelectorCreator<InputSelectors, Result>`

### Packaging changes

The package now ships a standard `exports` map. Consequences:

- **Deep imports into `dist` are no longer resolvable.** Only the package root and its `package.json` are exposed. Import from the package root instead of paths like `@veksa/re-reselect/dist/...`.
- **The UMD build moved to `dist/umd/index.umd.js`** (was `dist/umd/index.js`), and the `browser` field points at the new path. Direct CDN links that hard-code the old path (unpkg, jsDelivr, a `<script src>` copied from older docs) will 404 and must be updated.
- **The ESM build is now `dist/es/index.mjs`** (was `dist/es/index.js`), reached through the `import` condition, so native Node ESM loads it as a real ES module. The `module` field points at `dist/es/index.legacy-esm.js` for old bundlers that never look at `exports`.
- **Type declarations moved to `dist/es/index.d.ts`**, which both the `types` field and the `exports` map point at. Consumers resolving types through the package name are unaffected.

The importable value API (package root, CJS/ESM/UMD) is unchanged.
